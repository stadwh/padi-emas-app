...
export default function Home() {
  // State untuk berpindah halaman (Home, Tentang, Produk, Keranjang, Kontak)
  ...
  const checkoutToWhatsApp = () => {
    if (cart.length === 0 || !buyerName.trim() || !buyerAddress.trim()) {
      alert("Mohon lengkapi daftar belanjaan beserta detail nama & alamat!");
      return;
    }
    ...
    // Ganti nomor WhatsApp tujuan di bawah ini (gunakan awalan 62)
    const waNumber = "6281281997276";
    window.open(`https://api.whatsapp.com/send?phone=${waNumber}&text=${encodeURIComponent(text)}`, "_blank");
  };

  return (
    <div className="bg-[#FFFBF5] text-stone-800 flex flex-col min-h-screen selection:bg-amber-100 selection:text-amber-900">
      ...
      <nav className="bg-[#FFFBF5]/90 backdrop-blur-md sticky top-0 z-50 border-b border-stone-200/40">
        <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <button onClick={() => switchPage("home")} className="flex items-center space-x-2.5 cursor-pointer text-left group">
            <div className="w-9 h-9 rounded-full overflow-hidden border-2 border-amber-500/80 shadow-xs transition group-hover:scale-105">
              <img src="/logo padi emas.jpeg" alt="Logo" className="w-full h-full object-cover" />
            </div>
            <span className="font-serif font-extrabold text-xl text-stone-900 tracking-tight group-hover:text-emerald-700 transition">
              Padi Emas Nusantara
            </span>
          </button>
          ...
          <div className="hidden md:flex space-x-6 font-medium text-sm text-stone-600">
            {["home", "tentang", "produk", "keranjang", "kontak"].map((page) => {
              const labels = {
                home: "Home",
              ...
              return (
                <button
                  ...
                  className={`py-1 transition-all cursor-pointer relative ${
                    isActive ? "text-emerald-700 font-bold" : "text-stone-600 hover:text-stone-900"
                  }`}
                >
                  {labels[page]}
                </button>
          ...
          <button
            onClick={() => switchPage("produk")}
            className="bg-emerald-700 hover:bg-emerald-800 text-white font-bold px-5 py-2.5 rounded-full text-xs transition shadow-md shadow-emerald-700/10 tracking-wide cursor-pointer"
          >
            Pesan Sekarang
          </button>
        </div>
      ...
      <main className="flex-grow">
          ...
          <section className="py-16 md:py-24 px-6 max-w-6xl mx-auto transition-all">
            <div className="grid md:grid-cols-12 gap-12 items-center">
              <div className="md:col-span-7 space-y-6 text-left order-2 md:order-1">
                <div className="inline-flex items-center space-x-2 bg-amber-500/10 text-amber-800 text-xs font-bold px-3.5 py-1.5 rounded-full border border-amber-500/20">
                  <span>🌾</span>
                  <span className="tracking-wide">Beras Premium Pilihan</span>
                </div>

                <h1 className="font-serif text-4xl md:text-6xl font-black text-stone-900 leading-tight">
                  Beras Premium<br />
                  <span className="text-emerald-700 underline decoration-amber-400 decoration-wavy decoration-1">
                    Pilihan Keluarga
                  </span>
                </h1>
                ...
                <div className="pt-2 flex items-center gap-4">
                  <button
                    onClick={() => switchPage("produk")}
                    className="bg-emerald-700 text-white font-bold px-6 py-3.5 rounded-full shadow-lg shadow-emerald-700/20 hover:bg-emerald-800 transition text-sm cursor-pointer"
                  >
                    Lihat Menu
                  </button>
                  <button
                    onClick={() => switchPage("tentang")}
                    className="bg-white text-stone-800 border border-stone-200 font-bold px-6 py-3.5 rounded-full hover:bg-stone-50 transition text-sm shadow-xs cursor-pointer"
                  >
                    Kisah Kami
                  </button>
                </div>
              ...
              <div className="md:col-span-5 flex justify-center order-1 md:order-2 relative py-6">
                <div className="absolute w-72 h-72 bg-amber-500/10 blur-2xl rounded-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2"></div>
        ...
        {currentPage === "tentang" && (
          <section className="py-20 px-6 max-w-2xl mx-auto text-center space-y-6">
            <h2 className="font-serif text-3xl font-extrabold text-stone-900">Cerita Padi Emas</h2>
            <p className="text-stone-600 leading-relaxed text-sm md:text-base">
              Berawal dari Kepedulian Kecil. Padi Emas Nusantara lahir dari sebuah mimpi sederhana: membawa bulir beras terbaik, sejujur, dan sesehat mungkin langsung dari tangan para petani lokal pilihan di penjuru nusantara ke meja makan keluarga Anda.
            </p>
            <p className="text-stone-600 leading-relaxed text-sm md:text-base">
        ...
        {currentPage === "produk" && (
          <section className="py-16 px-6 max-w-6xl mx-auto">
            <div className="text-center max-w-xl mx-auto mb-16 space-y-2">
              <h2 className="font-serif text-3xl md:text-4xl font-extrabold text-stone-900 tracking-tight">Pilihan Produk</h2>
            ...
            <div className="grid md:grid-cols-3 gap-8">
              ...
              <div className="bg-white border border-stone-200/60 rounded-2xl overflow-hidden shadow-xs flex flex-col">
                ...
                <div className="p-6 flex-grow flex flex-col justify-between space-y-4">
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-stone-100">
                    <span className="font-extrabold text-base text-emerald-800">Rp 135.000</span>
                    <button onClick={() => addToCart("Beras Pandan Wangi", 135000)} className="bg-emerald-700 text-white text-xs font-bold px-4 py-2 rounded-full hover:bg-emerald-800 transition cursor-pointer">+ Tambah</button>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-stone-200/60 rounded-2xl overflow-hidden shadow-xs flex flex-col">
                ...
                <div className="p-6 flex-grow flex flex-col justify-between space-y-4">
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-stone-100">
                    <span className="font-extrabold text-base text-emerald-800">Rp 145.000</span>
                    <button onClick={() => addToCart("Beras Premium", 145000)} className="bg-emerald-700 text-white text-xs font-bold px-4 py-2 rounded-full hover:bg-emerald-800 transition cursor-pointer">+ Tambah</button>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-stone-200/60 rounded-2xl overflow-hidden shadow-xs flex flex-col">
                ...
                <div className="p-6 flex-grow flex flex-col justify-between space-y-4">
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-stone-100">
                    <span className="font-extrabold text-base text-emerald-800">Rp 155.000</span>
                    <button onClick={() => addToCart("Beras Ketan Putih", 155000)} className="bg-emerald-700 text-white text-xs font-bold px-4 py-2 rounded-full hover:bg-emerald-800 transition cursor-pointer">+ Tambah</button>
                  </div>
                </div>
              </div>
            </div>
        ...
        {currentPage === "keranjang" && (
          <section className="py-16 px-6 max-w-xl mx-auto">
            <h2 className="font-serif text-3xl font-extrabold text-stone-900 text-center mb-8">Keranjang Belanja</h2>
            <div className="bg-white border border-stone-200/60 rounded-2xl p-6 shadow-xs space-y-6">
              <div className="divide-y divide-stone-100">
                {cart.length === 0 ? (
                  <p className="text-stone-400 text-center py-8 text-xs">Keranjang Anda kosong pelong.</p>
                ) : (
                  cart.map((item, index) => (
              ...
              <div className="flex justify-between items-center font-bold text-base border-t pt-4 border-stone-100">
                ...
                <span className="text-emerald-800 text-xl">Rp {totalCartPrice.toLocaleString("id-ID")}</span>
              </div>
              ...
              <div className="space-y-3 pt-4 border-t border-stone-100">
                <input
                  ...
                  className="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-2.5 text-xs text-stone-800 focus:outline-none focus:border-emerald-700"
                />
                <textarea
                  rows="3"
                ...
                <button
                  ...
                  className="w-full bg-emerald-700 hover:bg-emerald-800 text-white font-bold py-3 rounded-xl transition flex items-center justify-center space-x-2 cursor-pointer text-xs"
                >
        ...
        {currentPage === "kontak" && (
          <section className="py-16 px-6 max-w-4xl mx-auto text-center space-y-4">
            <h2 className="font-serif text-3xl font-extrabold text-stone-900">Hubungi kami</h2>
            <p className="text-stone-500 text-xs">Operasional Toko: 09.00 - 20.00 WIB</p>
            <div className="max-w-sm mx-auto bg-white border border-stone-200/60 p-6 rounded-2xl text-xs text-stone-600 space-y-2 text-left">
